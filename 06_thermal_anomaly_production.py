...
def calculate_thermal_baseline(thermal_data):
    ...
    seasonal_baseline = (
        thermal_data.groupby("week_of_year")
...
def analyze_mine_site_thermal(site_name, features_list):
    ...
    for feature in features_list:
        thermal = fetch_modis_lst_data(
            feature["lat"], feature["lon"], "2022-01-01", "2024-01-01"
        )
        ...
        if feature["type"] == "waste_dump":
            ...
            thermal.loc[recent, "lst_day_celsius"] += np.random.normal(
                5, 2, recent.sum()
            )
        ...
        recent_period = anomalies["date"] > (
            anomalies["date"].max() - timedelta(days=90)
        )
        ...
        feature_summary = {
            ...
            "risk_level": "HIGH"
            if recent_anomalies["anomaly_score"].max() > 60
            else "MEDIUM"
            if recent_anomalies["anomaly_score"].max() > 40
            else "LOW",
        }
...
def analyze_thermal_trends(thermal_data, baseline, window_days=90):
    ...
    thermal_sorted["rolling_mean"] = (
        ...
        .rolling(window=window_days // 8, min_periods=3)
        .mean()
    )
...
def main():
    ...
    thermal_data["lst_night_celsius"] = kelvin_to_celsius(
        thermal_data["lst_night_kelvin"]
    )

    logger.info(f"   Collected {len(thermal_data)} observations")
    logger.info(
        f"   Temperature range: {thermal_data['lst_day_celsius'].min():.1f}°C to {thermal_data['lst_day_celsius'].max():.1f}°C"
    )
    ...
    recent_dates = thermal_with_anomaly["date"] > (
        thermal_with_anomaly["date"].max() - timedelta(days=90)
    )
    ...
    logger.info(
        f"   Maximum Severity Score: {anomalies['anomaly_score'].max():.1f}/100"
    )

    logger.info("\n4. Analyzing Multiple Mine Features...")
    mine_features = [
        {
            "name": "Main Tailings Dam",
            "type": "tailings_dam",
            "lat": -30.50,
            "lon": 121.50,
        },
        {
            "name": "North Waste Dump",
            "type": "waste_dump",
            "lat": -30.48,
            "lon": 121.52,
        },
        {
            "name": "South Waste Dump",
            "type": "waste_dump",
            "lat": -30.52,
            "lon": 121.48,
        },
    ...
    tailings_thermal["lst_day_celsius"] = kelvin_to_celsius(
        tailings_thermal["lst_day_kelvin"]
    )
    recent_mask = tailings_thermal["date"] > "2023-06-01"
    days_recent = (
        tailings_thermal.loc[recent_mask, "date"]
        - tailings_thermal.loc[recent_mask, "date"].min()
    ).dt.days
    ...
    logger.info(
        f"   Annual Trend: {trend_analysis['annual_trend_celsius']:+.2f}°C/year"
    )
