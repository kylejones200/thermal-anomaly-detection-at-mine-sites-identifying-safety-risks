...
def create_main_visualization(plot: bool = False):
    ...
    if plot:
        ...
        ax2.plot(
            anomalies["date"], anomalies["anomaly_score"], color="black", linewidth=1
        )
        ax2.axhline(
            y=40, color="gray", linestyle="--", linewidth=0.8, label="Medium Risk"
        )
        ax2.axhline(
            y=60, color="gray", linestyle="-.", linewidth=0.8, label="High Risk"
        )
        ...
        ax2.set_title(
            "Anomaly Severity Score", fontsize=12, fontweight="bold", loc="left"
        )
...
def create_trend_visualization(plot: bool = False):
    ...
    days_recent = (
        thermal_data.loc[recent_mask, "date"]
        - thermal_data.loc[recent_mask, "date"].min()
    ).dt.days
